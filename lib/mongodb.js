            ...
            if (spec === 'between') {
                ...
                query[k] = {
                    $in: cond.map(function (x) {
                        if (k == "id" || k == "_id") return ObjectID(x);
                        return x;
                    })
                };
            } else if (spec === 'nin') {
...
MongoDB.prototype.all = function all(model, filter, options, callback) {
    ...
    function processResponse(err, cursor) {
        ...
        cursor.toArray(function (err, data) {
            ...
            if (filter && filter.multiPointer && filter.multiPointer[model]) {
                self._models[model].model.multiPointer(objs, model, filter.includefilter, filter.multiPointer, next);
            }else{
                next();
            }
            function next() {
                if (filter && filter.include) {
                    self._models[model].model.include(objs, filter.include, options, callback,filter.includefilter,filter.multiPointer);
                } else {
                    callback(null, objs);
                }
            }
...
MongoDB.prototype.removeField =
    MongoDB.prototype.removeFieldAll = function updateAll(model, where, data, cb) {
        var self = this;
        if (self.debug) {
            debug('removeFieldAll', model, where, data);
        }
        var idName = this.idName(model);
        delete data[idName];
        self.db.collection(model).update(where, {$unset: data}, {multi: true, upsert: false},
            function (err, count) {
                if (self.debug) {
...
MongoDB.prototype.renameField = function updateAll(model, where, data, cb) {
    var self = this;
    if (self.debug) {
        debug('renameField', model, where, data);
    }
    var idName = this.idName(model);
    delete data[idName];
    self.db.collection(model).update(where, {$rename: data}, {multi: true, upsert: false},
        function (err, count) {
            if (self.debug) {
                debug('updateAll.callback', model, where, data, err, count);
            }
...
MongoDB.prototype.aggregate = function (model, filter, callback) {
    var self = this;
    if (self.debug) {
        debug('all', model, filter);
    }
    ...
    for(var i = 0,len = filter.length;i<len;i++){
        var stream = filter[i];
        if (stream.where) {
            if (stream.where[idName]) {
                var id = stream.where[idName];
                delete stream.where[idName];
                if (id.constructor !== Object) {
                    // {id: value}
                    id = self.coerceId(model, id);
                }
            ...
            stream["$match"] = self.buildWhere(model, stream.where);
            delete stream["where"];
        }
        ...
        if (stream.fields) {
            ...
            if (Array.isArray(stream.fields)) {
                stream.fields.forEach(function (val) {
                    project[val] = 1;
                })
            } else if (_.isPlainObject(stream.fields)) {
                Object.keys(stream.fields).forEach(function (key) {
                    if (_.isBoolean(stream.fields[key])) {
                        project[key] = stream.fields[key] ? 1 : -1;
                    } else {
                        project[key] = stream.fields[key];
                    }
                })
            }
        ...
        if(stream.order){
            var keys = stream.order;
            var order = {};
            if (typeof keys === 'string') {
                keys = keys.split(',');
            }
            for (var index = 0, zlen = keys.length; index < zlen; index++) {
                var m = keys[index].match(/\s+(A|DE)SC$/);
                var key = keys[index];
                key = key.replace(/\s+(A|DE)SC$/, '').trim();
                if (key === idName) {
        ...
        if(stream.expand){
            stream["$unwind"] = stream["expand"];
            delete stream["expand"];
        }
    }

    this.execute(model, 'aggregate', filter, processResponse);

    function processResponse(err, objs) {
        if (err) {
            return callback(err);
        }
        callback(null, objs);
...
MongoDB.prototype.buildNearFilter = function(query,near){
    ...
    if (!near.near) return;
    var sphere = near.near.split(',').map(function (k) {
        return parseFloat(k);
    }).reverse();
    where[near.key]={};
    if (near.maxDistance) {
        var maxDistance = near.maxDistance / EARTH_RADIUS[near.unit];
        where[near.key]["near"] = {"$near": sphere, "$maxDistance": maxDistance}
    } else {
        where[near.key]["near"] = {"$near": sphere}
    }
}

MongoDB.prototype.createIndex = function (model, fieldOrSpec, options, callback) {
    this.execute(model, 'createIndex', fieldOrSpec, options, callback);
}

MongoDB.prototype.dropIndex = function (model, indexName, callback) {
    this.execute(model, 'dropIndex', indexName, callback);
}